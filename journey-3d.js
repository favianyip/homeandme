...
const P_BASE = {
    hdb2: [{ W: 6.4, D: 7.4, rects: [ // traced from uploaded 2-room studio plan
      ...
      ['foyer', 3.4, 5.4, 3.0, 2.0, Object.assign({ lbl: 'FOYER', entry: 'N' }, PASS)],
    ] }],
    hdb3: [{ W: 9.4, D: 8.8, voids: [
      { key: 'notch_sw', x: 0, z: 7.0, w: 1.55, d: 1.8 }, { key: 'lobby', x: 7.15, z: 6.05, w: 1.575, d: 2.75 }, { key: 'lobby_e', x: 8.725, z: 6.05, w: 0.675, d: 2.75 },
    ], rects: [ // traced from uploaded '3 ROOM (POINT BLOCK) TYPE 2(H)' — 9400 × 8800
      ['ledge_n', 6.1, 0, 3.3, 0.45, { x: 1, out: 1, lbl: 'RC LEDGE' }],
      ...
      ['living', 6.1, 0.45, 3.3, 3.2, {}],
      ['hall2', 6.1, 3.65, 1.725, 2.4, Object.assign({ lbl: 'PASSAGE' }, PASS)],
      ['hs', 7.825, 3.65, 1.575, 2.4, { x: 1, lbl: 'HOUSEHOLD SHELTER' }],
      ...
      ['ac', 0, 6.05, 1.55, 0.95, { x: 1, out: 1, lbl: 'AC LEDGE' }],
      ['yard', 1.55, 6.05, 1.2, 2.75, { x: 1, lbl: 'SERVICE YARD' }],
      ['kitchen', 2.75, 6.05, 2.8, 2.75, { dw: 'E' }],
      ['foyer', 5.55, 6.05, 1.6, 2.75, Object.assign({ lbl: 'ENTRY', entry: 'N' }, PASS)],
    ] }],
    hdb4: [{ W: 11.7, D: 8.9, rects: [ // traced from uploaded '4-ROOM (CORRIDOR)' — 11700 × 8900, access balcony outside the envelope
      ['ledge_n', 0, 0, 6.15, 0.4, { x: 1, out: 1, lbl: 'RC LEDGE' }],
      ['common', 0, 0.4, 2.85, 4.1, { lbl: 'BEDROOM 2', dw: 'N' }],
      ['master', 2.85, 0.4, 3.3, 4.1, { lbl: 'MAIN BEDROOM', dw: 'N' }],
      ['ac', 6.15, 0, 1.7, 2.5, { x: 1, out: 1, lbl: 'AIR-CON LEDGE' }],
      ['yard', 7.85, 0, 1.6, 2.5, { x: 1, lbl: 'SERVICE YARD' }],
      ['bath_m', 6.15, 2.5, 1.7, 2.3, { pid: 'bath', lbl: 'BATH / WC 1', dw: 'N' }],
      ['bath_c', 7.85, 2.5, 1.6, 2.3, { pid: 'bath', lbl: 'BATH / WC 2', dw: 'N' }],
      ['ledge_e', 9.45, 0, 2.25, 0.4, { x: 1, out: 1, lbl: 'RC LEDGE' }],
      ['kitchen', 9.45, 0.4, 2.25, 6.0, { dw: 'W' }],
      ['corr', 0, 4.5, 2.85, 0.8, Object.assign({ lbl: 'HALL' }, PASS)],
      ['common2', 0, 5.3, 2.85, 3.6, { lbl: 'BEDROOM 3', dw: 'S' }],
      ['living', 2.85, 4.5, 3.3, 4.4, {}],
      ['living2', 6.15, 4.8, 3.3, 4.1, { pid: 'living', lbl: 'DINING', entry: 'N' }],
      ['store', 9.45, 6.4, 2.25, 2.5, { x: 1, lbl: 'STORE / PANTRY' }],
    ] }],
    d4: [{ W: 13.9, D: 9.5, voids: [
      { key: 'margin_w', x: 0, z: 0, w: 0.8, d: 9.5 }, { key: 'wall_balc_e', x: 5.645, z: 0, w: 0.355, d: 2.4 }, { key: 'chute', x: 0.8, z: 7.1, w: 0.8, d: 2.4 },
      { key: 'margin_s1', x: 1.6, z: 8.7, w: 2.9, d: 0.8 }, { key: 'margin_s2', x: 4.5, z: 8.5, w: 1.5, d: 1.0 }, { key: 'margin_s3', x: 6.0, z: 9.0, w: 6.4, d: 0.5 },
      { key: 'margin_e', x: 12.4, z: 5.6, w: 1.5, d: 3.9 },
    ], rects: [ // traced from uploaded 'Unit Layout Plan' (Private Condo Type D4)
      ['balcony', 0.8, 0, 4.845, 2.4, { x: 1, out: 1, lbl: 'BALCONY' }],
      ['kitchen', 0.8, 2.4, 2.1, 2.5, { dw: 'N' }],
      ['living', 2.9, 2.4, 3.1, 4.7, {}],
      ['foyer', 0.8, 4.9, 2.1, 2.2, Object.assign({ lbl: 'FOYER', entry: 'W' }, PASS)],
      ['hs', 1.6, 7.1, 1.3, 1.6, { x: 1, lbl: 'SHELTER' }],
      ['yard', 2.9, 7.1, 1.6, 1.6, { x: 1, lbl: 'SERVICE YARD & WC' }],
      ['ac', 4.5, 7.1, 1.5, 1.4, { x: 1, out: 1, lbl: 'AC LEDGE' }],
      ['common2', 6.0, 0, 2.6, 4.4, { lbl: 'BEDROOM 3', dw: 'N' }],
      ['common', 8.6, 0, 2.55, 4.4, { lbl: 'BEDROOM 2', dw: 'N' }],
      ['master', 11.15, 0, 2.75, 4.4, { lbl: 'MASTER BEDROOM' }],
      ['corr', 6.0, 4.4, 7.9, 1.2, Object.assign({ lbl: 'HALL' }, PASS)],
      ...
      ['ledge_s', 6.0, 8.0, 2.6, 1.0, { x: 1, out: 1, lbl: 'RC LEDGE' }],
      ['ledge_e', 8.6, 8.0, 3.8, 1.0, { x: 1, out: 1, lbl: 'AC LEDGE' }],
    ] }],
    hdb5: [{ W: 12.4, D: 9.0, rects: [ // catalog 5-room: 3 bed + dining band, corridor service core
      ['ledge_n', 0, 0, 6.4, 0.4, { x: 1, out: 1, lbl: 'RC LEDGE' }],
      ['common', 0, 0.4, 3.0, 4.1, { lbl: 'BEDROOM 2', dw: 'N' }],
      ['master', 3.0, 0.4, 3.4, 4.1, { lbl: 'MAIN BEDROOM', dw: 'N' }],
      ['ac', 6.4, 0, 1.7, 2.5, { x: 1, out: 1, lbl: 'AC LEDGE' }],
      ['yard', 8.1, 0, 1.8, 2.5, { x: 1, lbl: 'SERVICE YARD' }],
      ['bath_m', 6.4, 2.5, 1.7, 2.3, { pid: 'bath', lbl: 'BATH / WC 1', dw: 'N' }],
      ['bath_c', 8.1, 2.5, 1.8, 2.3, { pid: 'bath', lbl: 'BATH / WC 2', dw: 'N' }],
      ['ledge_e', 9.9, 0, 2.5, 0.4, { x: 1, out: 1, lbl: 'RC LEDGE' }],
      ['kitchen', 9.9, 0.4, 2.5, 6.0, { dw: 'W' }],
      ['corr', 0, 4.5, 3.0, 0.9, Object.assign({ lbl: 'HALL' }, PASS)],
      ['common2', 0, 5.4, 3.0, 3.6, { lbl: 'BEDROOM 3', dw: 'S' }],
      ['living', 3.0, 4.5, 3.4, 4.5, {}],
      ['living2', 6.4, 4.8, 3.5, 4.2, { pid: 'living', lbl: 'DINING', entry: 'N' }],
      ['store', 9.9, 6.4, 2.5, 2.6, { x: 1, lbl: 'STORE / UTILITY' }],
    ] }],
    hdbEA: [{ W: 16.1, D: 9.8, rects: [ // traced from uploaded Executive Apartment (point block, 146sqm)
      ...
      ['foyer', 14.2, 4.6, 1.9, 1.7, Object.assign({ lbl: 'ENTRY', entry: 'E' }, PASS)],
    ] }],
    c2: [{ W: 9.0, D: 8.6, rects: [
      ...
      ['foyer', 2.7, 7.4, 1.9, 1.2, Object.assign({ lbl: 'FOYER', entry: 'N' }, PASS)],
    ...
    c3: [{ W: 10.6, D: 8.8, rects: [
      ...
      ['foyer', 2.9, 7.5, 2.1, 1.3, Object.assign({ lbl: 'FOYER', entry: 'N' }, PASS)],
    ...
    cp: [{ W: 12.6, D: 9.2, rects: [
      ...
      ['foyer', 5.4, 6.2, 1.0, 3.0, Object.assign({ lbl: 'ENTRY FOYER', entry: 'N' }, PASS)],
    ...
    l3: [
      ...
      { W: 9.6, D: 11.0, rects: [
        ['terrace_n', 0, 0, 9.6, 2.4, { x: 1, lbl: 'ROOF TERRACE' }],
...
export const CATALOG_NAMES = {
  hdb2: 'HDB 2-ROOM FLEXI', hdb3: 'HDB 3-ROOM', hdb4: 'HDB 4-ROOM', hdb5: 'HDB 5-ROOM',
  hdbEA: 'HDB EXECUTIVE APARTMENT', d4: 'CONDO TYPE D4', c2: 'CONDO 2-BEDROOM',
  c3: 'CONDO 3-BEDROOM', cp: 'CONDO PENTHOUSE', sh1: 'TWO-STOREY SHOPHOUSE',
  l1: 'TERRACE HOUSE', l2: 'SEMI-DETACHED HOUSE', l3: 'DETACHED HOUSE',
};
export function catalogDocs() {
  const out = {};
  Object.entries(P_BASE).forEach(([key, storeys]) => {
    out[key] = {
      layout_key: key, display_name: CATALOG_NAMES[key] || key.toUpperCase(),
      trace_confidence: 'low', provenance: 'CATALOG LAYOUT — REPRESENTATIVE GEOMETRY',
      floors: storeys.map((s) => ({
        width_m: s.W, depth_m: s.D,
        voids: (s.voids || []).map((v) => Object.assign({}, v)),
        spaces: s.rects.map(([k, x, z, w, d, o]) => {
          o = o || {};
          const sp = { key: k, x, z, w, d };
          // o.x is the AUX flag in rect grammar — never copy it onto the coordinate
          if (o.x) sp.aux = 1;
          ['pid', 'lbl', 'pass', 'dw', 'open', 'rise', 'solid', 'entry', 'swout', 'out'].forEach((f) => { if (o[f] !== undefined) sp[f] = o[f]; });
          return sp;
        }),
      })),
    };
...
export function createModel(ctx) {
  ...
  const matBronze = M(0xb07c2a, 0.55, 'entry_threshold', { metalness: 0.25 });
  ...
  // Catalog geometry lives at module scope (P_BASE below); each build works on a private
  // copy so plan-document overrides in one createModel never leak into another.
  const P = JSON.parse(JSON.stringify(P_BASE));
  ...
  (ctx.planDocs || []).forEach((doc) => {
    ...
    P[doc.layout_key] = doc.floors.map((f) => ({ W: f.width_m, D: f.depth_m, voids: f.voids || [], rects: f.spaces.map((s) => {
      const o = {};
      if (s.aux !== undefined) o.x = s.aux; // aux = non-habitable flag (kept distinct from the x coordinate)
      ['pid', 'lbl', 'pass', 'dw', 'open', 'rise', 'nol', 'shop', 'solid', 'entry', 'swout', 'out'].forEach((k) => { if (s[k] !== undefined) o[k] = s[k]; });
  ...
  function storeyRects(st) {
    return { W: st.W, D: st.D, voids: st.voids || [], rects: st.rects.map(([key, x, z, w, d, o]) => ({ key, id: (o && o.pid) || key, x, z, w, d, o: o || {} })) };
  }
  ...
  function decide(b) {
    ...
    // one room traced as two rects (stepped living/dining) is ONE room, not two behind a wall
    if (r.id === n.id && r.id !== 'bath' && !rO.dw && !nO.dw && !rO.pass && !nO.pass && !rO.x && !nO.x) return { kind: 'open' };
    ...
    if (rO.x || nO.x) {
      ...
      // an RC / AC ledge is the outside of a facade: habitable rooms get their window onto it
      if (/^ledge/.test(xr.key)) return (other.o.x || other.o.pass) ? { kind: 'solid' } : other.id === 'bath' ? { kind: 'winsmall' } : { kind: 'window' };
      if (/^balcony/.test(xr.key)) return (other.id === 'living' || other.id === 'family' || other.id === 'master') ? { kind: 'slider' } : { kind: 'window' };
      if (/^terrace/.test(xr.key)) return { kind: 'slider' };
  ...
  function extKind(r, side, first) {
    ...
    if (k === 'foyer') return 'solid'; // the one main entrance is placed by placeEntry(), not per-room
    if (k === 'garden') return side === 'S' ? 'gate' : 'rail';
    if (/^ledge/.test(k)) return 'rail'; // open-air ledge: parapet, never a solid wall
    if (/^balcony/.test(k)) return 'railglass';
    if (/^terrace/.test(k) || k === 'ac') return 'rail';
  ...
  // Exactly one main entrance per unit, on the widest sensible external wall: a declared `entry`
  // side wins, then the foyer, then circulation / living / kitchen. Stands down when the unit is
  // already entered off a porch, footway or garden (that boundary is a 'frontdoor').
  function placeEntry(bs, si) {
    if (si !== 0 || bs.some((b) => b.dec.kind === 'frontdoor')) return;
    const OUTDOOR = ['garden', 'balcony', 'terrace', 'ac', 'yard'];
    const rank = (r) => r.o.entry ? 500 : r.key === 'foyer' ? 400 : r.o.pass ? 300
      : (r.id === 'living' || r.id === 'family') ? 200 : r.id === 'kitchen' ? 90 : 0;
    const cands = bs.filter((b) => {
      if (b.type !== 'ext' || b.b - b.a < 0.9) return false;               // needs a real leaf
      if (b.r.o.solid && b.r.o.solid.includes(b.side)) return false;       // party / blind wall
      if (b.r.o.entry && b.side !== b.r.o.entry) return false;             // respect the traced side
      return rank(b.r) > 0 && !OUTDOOR.includes(b.r.key);
    });
    cands.sort((p, q) => (rank(q.r) - rank(p.r)) || ((q.b - q.a) - (p.b - p.a)));
    const pick = cands[0];
    if (!pick) return;
    bs.forEach((b) => { if (b.dec.kind === 'entry') b.dec = { kind: 'solid' }; });
    pick.dec = { kind: 'entry', into: pick.r };
  }
  ...
  let dimsCur = false, roofNode = null;
  const matGhost = new THREE.MeshStandardMaterial({ name: 'ghost_upper', color: 0xb9c4bd, transparent: true, opacity: 0.1, roughness: 1, depthWrite: false });
  ...
  function drawBoundary(g, b, cx, cz, H) {
    ...
    const doorAt = (gw, leafMat, arc, openF) => {
      ...
      if (kind === 'entry' || kind === 'frontdoor') { // main entrance: threshold slab + ENTRANCE tag outside
        const mid = (gs + ge) / 2 - (horiz ? cx : cz), so = intoPos ? -1 : 1, o1 = 0.3 * so;
        const tx = horiz ? mid : base + o1, tz = horiz ? base + o1 : mid;
        doorNodes.push(mkBox(g, horiz ? gw + 0.12 : 0.5, 0.04, horiz ? 0.5 : gw + 0.12, matBronze, tx, 0.002, tz, nm('threshold'), false));
        labelNodes.push(makeSprite(g, 'ENTRANCE', horiz ? tx : tx + o1 * 1.9, 0.62, horiz ? tz + o1 * 1.9 : tz,
          { px: 36, color: 'rgba(176,124,42,0.98)', sx: 1.4, sy: 0.35, name: 'entry_tag', fade: true }));
      }
      if (arc) {
  ...
  function buildModel() {
    ...
    storeyGroups = []; dimNodes = []; furnNodes = []; doorNodes = []; labelNodes = []; bucket = null; roofNode = null;
    ...
    // (kept plain paper — the uploaded plan is only shown via the SOURCE PLAN toggle, never under mismatched layouts)
    const multi = dims.length > 1;
    // multi-storey: labels float INSIDE each level (above-wall spots are buried in the slab of the level above)
    const labY = multi ? Math.max(0.72, H.wall * 0.62) : H.wall + 0.5;
    dims.forEach((dim, si) => {
      ...
      sceneCur = { si, W: dim.W, D: dim.D, cx, cz, rects: dim.rects, voids: dim.voids || [], doors: [], bounds: [] };
      sceneData.storeys.push(sceneCur);
      if (multi) makeSprite(sg, 'LEVEL ' + (si + 1), -W / 2 - 1.15, Math.max(0.6, H.wall * 0.5), D / 2 + 0.7, { px: 46, color: 'rgba(90,126,138,0.98)', sx: 1.5, sy: 0.4, name: 'storey_badge' });
      if (si > 0) {
        ...
        if (si === dims.length - 1 && H.full) { // whole-building massing: roof over the top storey in FULL WALLS
          roofNode = mkBox(sg, bx1 - bx0 + 0.34, 0.1, bz1 - bz0 + 0.34, matSlab, (bx0 + bx1) / 2 - cx, H.wall + 0.02, (bz0 + bz1) / 2 - cz, 'roof_slab', true);
          roofNode.visible = !(state.view && state.view.cam === 'top');
        }
      }
      ...
      dim.rects.forEach((r) => {
        ...
        if (r.o.x) {
          ...
          labelNodes.push(makeSprite(sg, r.o.lbl || r.key.toUpperCase(), x, multi ? labY : labY - 0.34, z, { color: 'rgba(105,115,108,0.9)', fade: true }));
          return;
        }
        ...
        const ltxt = r.o.rise && si < dims.length - 1 ? 'STAIRS UP TO L' + (si + 2) : r.o.lbl || ROOMS[r.id].label;
        labelNodes.push(makeSprite(sg, ltxt, x, labY, z, { fade: true }));
  ...
  function setDims(v) { dimsCur = !!v; dimNodes.forEach((n) => { n.visible = dimsCur && !inGhost(n); }); }
  function furnFor(si) {
  ...
  function getScene() {
    ...
    return { storeys: sceneData.storeys.map((s) => ({
      si: s.si, W: s.W, D: s.D, cx: s.cx, cz: s.cz, voids: s.voids || [],
      rects: s.rects.map((r) => ({ key: r.key, id: r.id, x: r.x, z: r.z, w: r.w, d: r.d, pass: !!r.o.pass, xtra: !!r.o.x, out: !!r.o.out || r.key === 'ac' || /^ledge/.test(r.key), lbl: r.o.lbl || null })),
  ...
  function setDecor(patch) {
    ...
    furnNodes.forEach((n) => { n.visible = decor.furn && !inGhost(n); });
    doorNodes.forEach((n) => { n.visible = decor.swings && !inGhost(n); });
    labelNodes.forEach((n) => { n.visible = decor.labels && !inGhost(n); });
  }
  // Storey presentation: stacked (default), focus (L1/L2 chips — levels ABOVE the focused one
  // become a translucent ghost shell so a lower level's interior stays visible under the house
  // silhouette; levels below stay solid), or split — levels side by side on the ground.
  function inGhost(n) { let p = n; while (p) { if (p.userData && p.userData.ghostOn) return true; p = p.parent; } return false; }
  function setGhost(sg, on) {
    if (!!sg.userData.ghostOn === !!on) return;
    sg.userData.ghostOn = !!on;
    const ctl = new Set([...furnNodes, ...doorNodes, ...labelNodes, ...dimNodes]);
    sg.traverse((n) => {
      if (n === sg) return;
      if (n.name === 'storey_badge') return; // level tag stays readable on ghosted shells
      if (n.name === 'walk_overlay_storey') n.visible = !on;
      if (on && ctl.has(n)) n.visible = false;
      if (n.isMesh) {
        if (on) { if (!n.userData.gMat) { n.userData.gMat = n.material; n.userData.gCast = n.castShadow; } n.material = matGhost; n.castShadow = false; }
        else if (n.userData.gMat) { n.material = n.userData.gMat; n.castShadow = n.userData.gCast; delete n.userData.gMat; }
      } else if (n.isSprite || n.isLine) {
        if (on) { if (n.userData.gVis == null) n.userData.gVis = n.visible; n.visible = false; }
        else if (n.userData.gVis != null) { n.visible = n.userData.gVis; delete n.userData.gVis; }
      }
    });
    if (!on) { setDims(dimsCur); setDecor({}); }
  }
  function applyStoreyLayout(instant) {
    ...
    storeyGroups.forEach((sg, si) => {
      sg.visible = true;
      setGhost(sg, solo >= 0 && si > solo);
      const tx = split ? si * (Wcur + 1.7) : 0;
      const ty = split ? 0 : si * SHcur;
      if (instant) { sg.position.x = tx; sg.position.y = ty; }
      else { tween(sg.position, 'x', tx, 550); tween(sg.position, 'y', ty, 550); }
    });
  }
  function refreshRoof() { if (roofNode) roofNode.visible = !(state.view && state.view.cam === 'top'); }
  ...
  function pick(nx, ny, camera) {
    ...
    const floors = [];
    storeyGroups.forEach((sg) => { if (sg.visible && !sg.userData.ghostOn) sg.children.forEach((c) => { if (c.userData && c.userData.rid) floors.push(c); }); });
  ...
  return { buildModel, markActive, tintRoom, setDims, setDecor, setStorey, setSplit, storeyCount, refreshRoof, pick, getBounds, getScene, setOverlay, clearOverlay, setUnderlay, planDoc: (k) => PLAN_DOCS[k] || null };
}
