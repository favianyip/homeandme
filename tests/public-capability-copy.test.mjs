...
test('active public pages describe service-gated review, not browser-authored measured geometry', async () => {
  ...
  assert.match(projectJourneyUi, /all \$\{contract\.previews\.length\} manifest-bound reference views before approval/i);
  assert.match(projectJourneyUi, /Legacy artifacts remain inspectable, but approval requires a regenerated complete-room coverage ledger/i);
  ...
  assert.match(modelArtifacts, /previewViews and referenceViewCoverage must either both be present or both be absent/);
  assert.match(modelArtifacts, /referenceViewCoverage must declare no uncovered canonical rooms/);
  assert.match(modelArtifacts, /must contain exactly the seven bound metadata fields/);
  assert.match(modelArtifacts, /roomName is not safe canonical display text/);
  assert.match(modelArtifacts, /roomFunction is invalid/);
  assert.match(modelArtifacts, /expected\.length === 1 \+ contract\.previews\.length/);
  assert.match(modelArtifacts, /approvalEligible = ready && contract\?\.mode === 'dynamic'/);
  assert.match(projectJourneyUi, /Function · \$\{descriptor\.roomFunction\}/);
